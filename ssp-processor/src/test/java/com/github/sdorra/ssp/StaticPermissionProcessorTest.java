...
public class StaticPermissionProcessorTest {
    ...
    final JavaFileObject expectedOutput = JavaFileObjects.forSourceString(
            ...
            Joiner.on(System.lineSeparator()).join(
                    ...
                    "import com.github.sdorra.ssp.DefaultPermissionCheck;",
                    "import com.github.sdorra.ssp.DefaultPermissionActionCheck;",
                    ...
                    "    return new DefaultPermissionActionCheck<>(TYPE.concat(Constants.SEPARATOR).concat(action));",
                    "  }",
                    "",
                    "  private static PermissionCheck check(String permission) {",
                    "    return new DefaultPermissionCheck(TYPE.concat(Constants.SEPARATOR).concat(permission));",
                    "  }",
                    "}"
            )
    );
